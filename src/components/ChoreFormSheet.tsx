...
function ChoreFormBody({ initial, onClose, defaultBuddyUid }: {
  ...
  // For per-buddy spawn, default the date to today so the chore is due
  // immediately. Otherwise (pool-creation), leave null so the user picks.
  const [onceDate, setOnceDate] = useState<Date | null>(defaultBuddyUid ? new Date() : null);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      ...
      <RecurrencePicker
        ...
        onConfirm={(r) => {
          ...
          else if (defaultBuddyUid && !onceDate) setOnceDate(new Date());
        }}
      />
