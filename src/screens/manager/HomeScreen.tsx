...
export default function HomeScreen({ navigation }: any) {
  ...
  const goChores = () => drawer?.navigate('ChorePool');
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      <ScrollView contentContainerStyle={{ padding: theme.spacing.lg }}>
        <TouchableOpacity
          style={[s.card, totalPending > 0 && { borderColor: theme.colors.danger + '80', backgroundColor: '#1e0e0e' }]}
          onPress={() => goChores()}
        >
          <Text style={[s.bigNum, { color: totalPending > 0 ? theme.colors.danger : theme.colors.success }]}>{totalPending}</Text>
          <View style={{ flex: 1 }}>
        ...
        {buddies.length > 0 && (
          <>
            <Text style={s.sectionLabel}>Buddies</Text>
        ...
        <TouchableOpacity style={s.card} onPress={goChores}>
