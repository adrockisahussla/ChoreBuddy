...
export default function RewardPoolScreen({ navigation }: any) {
  ...
  const filtered = filterKid === 'all'
    ? rewardPool
    : rewardPool.filter(p => p.kidId === filterKid);
        ...
        {buddies.length > 0 && (
          <View style={s.filterRow}>
            <Pill
              label="All"
              size="sm"
              active={filterKid === 'all'}
              onPress={() => setFilterKid('all')}
            />
            {buddies.map(b => (
              <Pill
                key={b.uid}
                label={b.displayName}
                size="sm"
                active={filterKid === b.uid}
                onPress={() => setFilterKid(b.uid)}
              />
            ))}
          </View>
        )}
        ...
        {filtered.length === 0 ? (
          <Text variant="empty" style={{ padding: 30 }}>
            {rewardPool.length === 0
              ...
              : 'No rewards for this kid yet.'}
          </Text>
        ) : (
          filtered.map(p => {
            const kid = buddies.find(b => b.uid === p.kidId);
            return (
...
function RewardPoolForm({ initial, onClose }: { initial: RewardPoolItem | null; onClose: () => void }) {
  ...
  const canSave = !!kidUid && minutes > 0 && pointsCost > 0;
      ...
      >
        ...
        <TouchableOpacity style={s.fieldBtn} onPress={() => setPickerOpen(true)}>
          ...
          <RNText style={s.fieldText} numberOfLines={1}>
            {selectedKid?.displayName || 'Pick a kid'}
          </RNText>
        ...
        <View style={s.formFooter}>
          <Button
            label={editing ? 'Save changes' : 'Add to pool'}
            variant="primary"
            full
            onPress={save}
            disabled={!canSave}
          />
          {editing && (
            <Button label="🗑 Delete reward" variant="danger" onPress={del} full style={{ marginTop: 8 }} />
          )}
        </View>
      </KeyboardAwareScrollView>
            ...
            {buddies.length === 0 ? (
              ...
              buddies.map(b => (
                <TouchableOpacity
                  key={b.uid}
                  style={s.sheetRow}
                  onPress={() => { setKidUid(b.uid); setPickerOpen(false); }}
                >
